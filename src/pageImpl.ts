...
export function sendInitialData(wizardName: string, data: Map<string, string>) {
  ...
  if (panel) {
    ...
    let obj = Array.from(data).reduce(
...
export function updatePanelTitle(name: string, title: string) {
...
export function createOrShowWizardWithPaths(
  ...
  if (panel) {
    ...
    panel = vscode.window.createWebviewPanel(viewType, title, vscode.ViewColumn.One, {
      ...
      localResourceRoots: [context.extensionUri, rootPathUri, vscode.Uri.joinPath(context.extensionUri, 'resources')],
    });
    ...
    // Load a Global Style for all pages
    // By default, the CSS file is located in the extension folder under wizardtemplates/<webview id>/all.css
    const extensionUriString = pagesWebviewUri.toString() + '/../../../../wizardtemplates/' + id + '/all.css';
    ...
    panel.onDidDispose(() => currentPanels.delete(id), undefined, context.subscriptions);
    currentPanels.set(id, panel);
  }
}

function createDispatch(messageMappings: MessageMapping[], currentPanelName: string, resourceRoot: string) {
...
const postMessageHandlerResponse = async (
  ...
  if (templates) {
    ...
    templates.forEach((template) => {
      if (template.content) {
        ...
        response.contents?.push({
          id: template.id,
          body: handlebars.compile(fs.readFileSync(path.join(resourceRoot, template.contentUrl)).toString())(result),
        });
      }
    });
  } else {
